...
class RestaurantDifferenceCalculator:
    def __init__(self):
        self.window = None
        self.text_area = None
        self.filtered_restaurants = []  # 儲存篩選後的餐廳名稱
    ...
    def calculate_difference(self):
        try:
            ...
            # 確保兩個檔案都有 '餐廳名稱' 欄位
            if '餐廳名稱' not in df1.columns or '餐廳名稱' not in df2.columns:
                messagebox.showerror("錯誤", "Excel檔案中缺少 '餐廳名稱' 欄位")
                return

            # 取得兩個檔案的餐廳名稱集合
            set1 = set(df1['餐廳名稱'].astype(str))
            set2 = set(df2['餐廳名稱'].astype(str))

            # 計算差集（在set1中但不在set2中的餐廳）
            difference = sorted(list(set1 - set2))
            self.filtered_restaurants = difference  # 儲存篩選結果
    ...
    def create_result_window(self, difference):
        ...
        # 添加標籤
        label = ttk.Label(frame, text=f"找到 {len(difference)} 個未重複的餐廳：")
        ...
        # 插入結果
        for restaurant in difference:
            self.text_area.insert(tk.END, f"{restaurant}\n")
    ...
    def export_filtered_data(self):
        try:
            if not self.filtered_restaurants:
            ...
            try:
                # 查詢資料
                query = """
                SELECT DISTINCT 餐廳編號, 餐廳名稱
                FROM menu_items
                WHERE 餐廳名稱 = ?
                """
                results = []
                for restaurant in self.filtered_restaurants:
                    db.cursor.execute(query, (restaurant,))
                ...
                default_filename = f"restaurant_filtered_export_{current_time}.csv"
                ...
                with open(file_path, 'w', encoding='utf-8-sig', newline='') as f:
                    ...
                    # 寫入資料列
                    for row in results:
                        writer.writerow([str(item).strip() if item is not None else '' for item in row])
